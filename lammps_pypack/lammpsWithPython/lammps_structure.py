...
## General TODO:
## bond, frame, atom - Refactor for more robust/clearer bond and atom type handling
## frame, node, atom - Refactor constraints in a more lammps-friendly format
## bond, frame - Create function to calculate and format bond parameters (use for add_node_bonds too)
...
class frame:
    ...
    def __str__(self) -> str:
        ...
        print_str += f'\n\nNumber of Atoms in Frame: {len(self.atoms)}\n'
        return print_str
    ...
    def transform_affine(self, transform_matrix: np.ndarray):
        ...
        return NotImplemented #new_frame
    ...
    def discretize(self, atom_distance: float, start_atom_id: int = 1, start_bond_id: dict | None = None) -> tuple:
        ...
        # Discretize the frame's nodes and elements into atoms
...
class node:
    ...
    '''
    def __init__(self, parent_frame: frame, coords: np.ndarray | None = None, constraint: np.ndarray | None = None):
    ...
    def edit_coords(self, new_coords: np.array):
        ...
        for el in self.elements:
            ...
            self.L = self.calc_arc_length(para_eq, is_linear=self.para_linear)
...
class element:
    ...
    def __init__(
        ...
        self.L = self.calc_arc_length(para_eq=para_eq, is_linear=self.para_linear)
    ...
    def __str__(self):
        ...
        print_str += f', length {self.L}, Is_Linear {self.para_linear}'
    ...
    def calc_arc_length(self, is_linear: bool, para_eq: list[callable], bounds: list | None = None, n_pts: int = 200) -> float:
        '''
        ...
        if is_linear is False:
            ...
            L = np.linalg.norm(self.node_b.coords - self.node_a.coords)
        return L
    ...
    def set_material(self, mat: list | material | str):
        if isinstance(mat, list):
            ...
            mat = next(filter(lambda mat_entry: mat_entry.name == mat, self.parent_frame.materials), None)
    ...
    def discretize(self, atom_distance: float) -> tuple:
        ...
        N_bonds = int(np.ceil(self.L/atom_distance))
    ...
    def __init__(
        ...
        *,
        ...
        if not parent_node is None:
            ...
            parent_frame = parent_node.parent_frame
        if not parent_frame is None:
            ...
            for atm_type in parent_frame.atom_type_list:
                if diameter == atm_type[1] and density == atm_type[2]:
                    self.type_id = atm_type[0]
            if self.type_id is None:
                self.type_id = len(parent_frame.atom_type_list)
                parent_frame.atom_type_list.append([self.type_id, diameter, density])
            parent_frame.atoms.append(self)

        ## TODO - Handle atom_type's of atoms not belonging to node or element

        self.id = N_previous_atoms

## TODO - use frame.bond_type_list to reference type and properties - to remove attributes from bond object
class bond:
    ...
    def __init__(
        ...
        *,
        ...
        if not parent_node is None:
            ...
            parent_frame = parent_node.parent_frame
        if not parent_frame is None:
            existing_bonds = [bnd for bnd in parent_frame.bonds if bnd.type == self.type]
            if not self.type in parent_frame.start_bond_id:
                parent_frame.start_bond_id[self.type] = 1
            N_previous_bonds = parent_frame.start_bond_id[self.type] + len(existing_bonds)

            # Calculate and assign next type id (distinct combination of type and parameters)
            for bnd_type in parent_frame.bond_type_list:
                parameters_match = self.compare_lists(self.parameters, bnd_type[2])
                if self.type == bnd_type[1] and parameters_match:
                    self.type_id = bnd_type[0]
            if self.type_id is None:
                self.type_id = 1 + len([bnd_typ for bnd_typ in parent_frame.bond_type_list if bnd_typ[1] == self.type])
                parent_frame.bond_type_list.append([self.type_id, self.type, self.parameters])
            parent_frame.bonds.append(self)
    ...
    def compare_lists(self, list_a: list, list_b: list) -> bool:
        lists_match = True
        if list_a is None or list_b is None:
            ...
            for i, val_a in enumerate(list_a):
                ...
                if not type(val_a) is type(val_b):
                    ...
                    lists_match = self.compare_lists(val_a, val_b)
